<?php
	...
	$featured_category = get_cat_ID('featured');
	...
	if(isset($pages[0])) {
		$all_url = get_page_link($pages[0]->ID);
	}
	$featured_query = new WP_Query(
		array(
			...
			'posts_per_page' => 1,
			'post_status' => 'publish',
			'category__in' => array( $featured_category )
		)
	);
...
?>
	<div class="blog-list grid-x align-center">
		<div class="cell small-12 blog-list__featured-container <?= $nofeat; ?>">
			<h1 class="blog-list__title text-center <?= $nofeat; ?>"><?php the_title(); ?></h1>
			<div class="grid-container blog-list__featured-wrapper">	
			<?php
				...
				if ( $featured_query->have_posts() ) : while ( $featured_query->have_posts() ) : $featured_query->the_post();
					...
					$content = wpautop( wp_trim_words( get_the_content(), 15, '...' ) );
			...
			?>
				<div class="blog-list__featured grid-x align-center">
					<div class="cell small-12 medium-6 large-7">
						<div class="blog-list__featured-image" style="background-image: url('<?= $image['url']; ?>');">
						<picture>
							<img class="visuallyhidden lazy" data-src="<?= $image['url']; ?>" alt="<?= $image['url']; ?>">
						</picture>
						</div>
					</div>
					<div class="blog-list__featured-content cell small-12 medium-6 large-5">
						<h2 class="blog-list__featured-category"><?= $category; ?><span><?= do_shortcode('[rt_reading_time label= "&nbsp;|&nbsp;" postfix="MIN READ" postfix_singular="MIN READ" post_id="' . $post->ID . '"]'); ?></h2>
						<h3 class="blog-list__featured-title"><?php the_title(); ?></h3>
						<div class="blog-list__featured-meta">
						<span class="blog-list__featured-date"><?= $date; ?></span><span class="blog-list__featured-author">By <?= $author; ?></span>
						</div>
					</div>
		...
		<div class="blog-list__body-container grid-container">
			<div class="blog-list__filter grid-x align-spaced align-mddle">
				<div class="blog-list__filter-categories-container cell xlarge-7 medium-7 small-12">
					...
					<li class="blog-list__filter-categories-item"><a href="<?= $all_url; ?>" class="selected"><span>All</span></a></li>
					<?php
					...
					?>	  
							<li class="blog-list__filter-categories-item"><a href="<?= $cat_link; ?>"><span><?= $cat_filter->cat_name; ?></span></a></li>   
